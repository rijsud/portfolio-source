        ...
        {navLinks.map((link) => {
          ...
          return (
            <li key={link.href}>
              ...
              >
                {/* 
                  The indicator bullet: 
                  If active, it forces the w-8 (extended) and lighter color.
                  If inactive, it stays w-1 and relies on group-hover for the interaction.
                */}
                <span
                  className={`mr-4 h-1 rounded-full transition-all motion-reduce:transition-none ${
                    isActive
                      ? "w-8 bg-accent"
                      : "w-1 bg-text-muted group-hover:w-8 group-hover:bg-accent"
                  }`}
                ></span>
                ...
                */}
                <span
                  className={`nav-text text-xs font-bold uppercase tracking-widest ${
                    isActive
                      ? "text-accent"
                      : "text-text-muted group-hover:text-accent group-focus-visible:text-accent"
                  }`}
                >
                  {link.name}
                </span>
